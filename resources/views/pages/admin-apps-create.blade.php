    ...
    <div class="card my-4 mx-5 rounded-4 shadow-lg">
        <div class="card-body">
            ...
            <div class="mx-auto" style="width: 60%">
                <h1 class="d-flex justify-content-center fs-4 mt-5 fw-bold">Tambah Data Aplikasi</h1>
                <form method="POST" action="{{ route('admin.apps-store') }}" class="p-3">
                    @csrf

                    <!-- App Code -->
                    <div class="mb-3">
                        <label for="app_code" class="form-label">{{ __('Kode App') }}</label>
                        <input id="app_code" class="form-control" type="text" name="app_code" :value="old('app_code')"
                            required autofocus />
                        @error('app_code')
                    ...
                    <div class="mb-3">
                        ...
                        <input id="app_name" class="form-control" type="text" name="app_name" :value="old('app_name')"
                            required autofocus />
                        @error('app_name')
                            <div class="text-danger mt-2">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <input id="app_group" class="form-control" type="text" name="app_group" :value="old('app_group')"
                            required autofocus />
                        @error('app_group')
                            <div class="text-danger mt-2">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <input id="app_url" class="form-control" type="text" name="app_url" :value="old('app_url')"
                            required autofocus />
                        @error('app_url')
                            <div class="text-danger mt-2">{{ $message }}</div>
                        @enderror
