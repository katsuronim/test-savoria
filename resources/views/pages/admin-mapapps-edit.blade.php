    ...
    <div class="card my-4 mx-5 rounded-4 shadow-lg">
        <div class="card-body">
            ...
            <div class="mx-auto" style="width: 60%">
                <h1 class="d-flex justify-content-center fs-4 mt-5 fw-bold">Ubah Data Hak Akses Pengguna</h1>
                <form method="POST" action="{{ route('admin.map-apps-user-update', $maps->id) }}">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" name="app_id" id="app_id">
                            ...
                            @foreach ($apps as $item)
                                <option value="{{ $item->app_id }}" {{ $maps->app_id == $item->app_id ? 'selected' : '' }}>
                        ...
                        <select class="form-select" name="user_id" id="user_id">
                            ...
                            @foreach ($users as $item)
                                <option value="{{ $item->user_id }}"
                                    {{ $maps->user_id == $item->user_id ? 'selected' : '' }}>
                                    {{ $item->user_code }} - {{ $item->user_fullname }}
                                </option>
                            @endforeach
                        </select>
                        @error('user_id')
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" name="data_status" id="data_status">
                            <option disabled selected></option>
                            <option value="1" {{ $maps->data_status == 1 ? 'selected' : '' }}>Aktif</option>
                            <option value="0" {{ $maps->data_status == 0 ? 'selected' : '' }}>Tidak Aktif</option>
                        </select>
                        @error('data_status')
                            <div class="text-danger mt-2">{{ $message }}</div>
                        @enderror
